...
// ROCmModelProfile is the runtime-facing model registry result. Portable builds
// resolve it from model-pack metadata so CPU/CUDA/legacy binaries expose the
// same reactive API surface as native ROCm builds.
...
func rocmResolvePortableModelProfile(path string, model inference.ModelIdentity) ROCmModelProfile {
