...
// TestComposedTextModel_DeclaredChatTemplate_ChatML proves the real serve wrap
// (composedTextModel, built by metalBackend.LoadModel for every composed/hybrid
...
func TestComposedTextModel_DeclaredChatTemplate_ChatML(t *testing.T) {
	for _, mt := range []string{"qwen3_5", "qwen3_5_moe", "qwen3_6", "qwen3_6_moe", "qwen3_next"} {
		m := &composedTextModel{modelType: mt}
...
// TestComposedTextModel_DeclaredChatTemplate_GemmaFallback proves a non-qwen
...
func TestComposedTextModel_DeclaredChatTemplate_GemmaFallback(t *testing.T) {
