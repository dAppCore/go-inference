...
// sharing the chain's one pooled gd scratch (both weight forms cast to bf16 at the
// qmv/gemv boundary).
...
// the AttnQuantFullLayerDevice body against the ping-pong hidden.
...
func attnQuantChainMoELayerDevice(ctxAny, dev any, inputNorm []float32, qw, kw, vw, ow *model.QuantWeight, qNormW, kNormW, postNorm []float32, moe *chainMoE, priorK, priorV []float32, H, KVH, HD, RD, pos0, window, gated, qkNorm int, eps, theta float32) (any, error) {
	ctx, ok := ctxAny.(*composedChainCtx)
	if !ok {
...
func gatedDeltaQuantChainMoELayerDevice(ctxAny any, sc *attn.GatedDeltaScratch, inputNorm []float32, w *model.GatedDeltaWeights, cfg model.GatedDeltaConfig, postNorm []float32, moe *chainMoE, priorConv, priorDelta []float32, eps float32) error {
	ctx, ok := ctxAny.(*composedChainCtx)
	if !ok {
