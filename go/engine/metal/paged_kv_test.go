...
func TestDevicePagedKVAttentionHalfMatchesContiguousStep(t *testing.T) {
	...
	for pos, x := range inputs {
		...
		if enc, _, err = encAttnHalfKVPaged(enc, cb, nil, false, residentBytes(x), cache, scalarI32(int32(pos)), outBuf, 0, attnNorm, bufView{}, bufView{}, bufView{}, nil, *sc, proj, dModel, nHeads, nKVHeads, headDim, pos, 0, headDim, base, scale, scale, eps, nil); err != nil {
			endEncodingFast(enc)
...
func TestDevicePagedKVSharedAttentionMatchesLinearShared(t *testing.T) {
	...
	for i, x := range inputs {
		...
		cb := commandBufferFast(queue)
		enc := computeCommandEncoderFast(cb)
		if enc, _, err = encAttnHalfKVPaged(enc, cb, nil, false, residentBytes(x), cache, scalarI32(int32(i)), ownerOut, 0, ownerNorm, bufView{}, bufView{}, bufView{}, nil, *ownerScratch, ownerProj, dModel, nHeads, nKVHeads, headDim, i, 0, headDim, base, scale, scale, eps, nil); err != nil {
			endEncodingFast(enc)
	...
	cb = commandBufferFast(queue)
	enc = computeCommandEncoderFast(cb)
	if err := encAttnHalfSharedPaged(enc, residentBytes(sharedX), cache, scalarI32(int32(pos)), gotBuf, 0, sharedNorm, bufView{}, bufView{}, *sharedScratch, sharedProj, dModel, nHeads, nKVHeads, headDim, pos, 0, headDim, base, scale, scale, eps, nil); err != nil {
		endEncodingFast(enc)
