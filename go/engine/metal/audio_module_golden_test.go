...
// audio_module_golden_test.go pins the native Conformer tower to HF Gemma4AudioModel goldens computed
// on the REAL e2b-4bit BF16 tower (weights loaded from the local HF cache, conv layouts adapted). It
// skips when the checkpoint is absent (supervised parity, not a portable CI gate). The synthetic mel is
// shared byte-for-byte via the fixture's bf16 bytes, so only tower-math rounding remains. The two
// defect-catchers are first: the subsample output collapses if the OHWI conv is double-transposed, and
// the tower output is off by the [1536] bias (max|abs| 14.875) if output_proj.bias is dropped.

type audioModuleGolden struct {
...
// TestAudioTowerModuleGoldens runs the assembled native tower on the shared synthetic mel and pins its
// per-module outputs to the HF goldens at cosine >= 0.999. Skips without the local checkpoint.
func TestAudioTowerModuleGoldens(t *testing.T) {
	...
	weights, cfg, _, ok := nativeAudioFromLoaded(m.audio, g.Frames, g.MelBins)
	if !ok {
		t.Fatal("nativeAudioFromLoaded failed")
	}
	...
	// Bias-applied, crisply: a dropped output_proj.bias would blow max|Δ| to ~14.875 (the bias
	// max|abs|) and cosine to 0.9837; 2.0 leaves ample headroom over the observed GPU-GEMM rounding.
