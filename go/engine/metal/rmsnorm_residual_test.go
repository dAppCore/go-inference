...
// TestRmsnormResidual_RMSNormResidualBF16_Good is the numerical gate for the
// fused rms-norm+residual kernel: out = res + RMSNorm(x, w) computed in one
// dispatch must track AddBF16(res, RMSNormBF16(x, w)) — the composed
// rms→bf16→add→bf16 path — across gemma hidden/head sizes. NOT byte-identical:
// a native-Metal `bfloat` kernel rounds tie-cases differently from MLX's
// bfloat16_t (its bf16 kernels), so ~10% of elements differ by ~1 ULP — the
// SAME bf16-rounding gap the fused gelu kernel documents. It is numerically
// equivalent (cosine ~1.0); fusing it onto the decode is therefore a
// deliberate "fp32-internal, lockstep both engines" numerics decision, not a
// free byte-identical swap. This test pins that it stays numerically tight
// and quantifies the gap, rather than asserting an unachievable bit-equality.
func TestRmsnormResidual_RMSNormResidualBF16_Good(t *testing.T) {
...
// TestRmsnormResidual_RMSNormResidualBF16_Bad pins the shape validation: x,
// res and weight must each be axisSize bf16 bytes, each mismatch rejected with
// an error rather than a mis-shaped dispatch.
func TestRmsnormResidual_RMSNormResidualBF16_Bad(t *testing.T) {
	requireNativeRuntime(t)
	if !gpuHasGeluKernel() {
		t.Skip("custom kernel library (lthn_kernels.metallib) not loaded")
	}
	...
	const axisSize = 256
	x, w, res := rmsNormResidualFixture(axisSize)
	if _, err := RMSNormResidualBF16(x[:2], w, res, axisSize, 1e-6); err == nil {
		t.Fatal("expected RMSNormResidualBF16 to reject short x")
	}
	if _, err := RMSNormResidualBF16(x, w, res[:2], axisSize, 1e-6); err == nil {
		t.Fatal("expected RMSNormResidualBF16 to reject short res")
	}
	if _, err := RMSNormResidualBF16(x, w[:2], res, axisSize, 1e-6); err == nil {
		t.Fatal("expected RMSNormResidualBF16 to reject short weight")
	}
}

// TestRmsnormResidual_RMSNormResidualBF16_Ugly pins the single-row kernel
// ceiling: a well-formed row wider than rmsLoopedLimit has no looped fused
// variant, so it must be rejected — not dispatched beyond the threadgroup cap
// (the failure shape the plain RMSNorm's looped lane exists to avoid).
func TestRmsnormResidual_RMSNormResidualBF16_Ugly(t *testing.T) {
	requireNativeRuntime(t)
	if !gpuHasGeluKernel() {
		t.Skip("custom kernel library (lthn_kernels.metallib) not loaded")
	}

	const axisSize = rmsLoopedLimit + 64
	x, w, res := rmsNormResidualFixture(axisSize)
	if _, err := RMSNormResidualBF16(x, w, res, axisSize, 1e-6); err == nil {
		t.Fatal("expected RMSNormResidualBF16 to reject an axis beyond the single-row kernel limit")
	}
}

// TestRmsnormResidual_RMSNormResidualBF16Into_Good pins the Into contract: the
// caller-owned output backing is used directly and the bytes are identical to
// the allocating wrapper.
func TestRmsnormResidual_RMSNormResidualBF16Into_Good(t *testing.T) {
	requireNativeRuntime(t)
	if !gpuHasGeluKernel() {
		t.Skip("custom kernel library (lthn_kernels.metallib) not loaded")
	}
...
// TestRmsnormResidual_RMSNormResidualBF16Into_Bad pins the shape validation
// through the Into surface: the same x/res/weight checks fire regardless of
// the output the caller offers.
func TestRmsnormResidual_RMSNormResidualBF16Into_Bad(t *testing.T) {
	requireNativeRuntime(t)
	if !gpuHasGeluKernel() {
		t.Skip("custom kernel library (lthn_kernels.metallib) not loaded")
	}
	...
	x, w, res := rmsNormResidualFixture(axisSize)
	out := make([]byte, axisSize*bf16Size)
	if _, err := RMSNormResidualBF16Into(out, x[:2], w, res, axisSize, 1e-6); err == nil {
		t.Fatal("expected RMSNormResidualBF16Into to reject short x")
	}
	if _, err := RMSNormResidualBF16Into(out, x, w[:2], res, axisSize, 1e-6); err == nil {
		t.Fatal("expected RMSNormResidualBF16Into to reject short weight")
	}
}

// TestRmsnormResidual_RMSNormResidualBF16Into_Ugly pins the fallback
// allocation contract: a nil or under-capacity out cannot be reused, so the
// call allocates a fresh row — still byte-identical to the allocating wrapper.
func TestRmsnormResidual_RMSNormResidualBF16Into_Ugly(t *testing.T) {
	requireNativeRuntime(t)
	if !gpuHasGeluKernel() {
		t.Skip("custom kernel library (lthn_kernels.metallib) not loaded")
	}
	...
	fromNil, err := RMSNormResidualBF16Into(nil, x, w, res, axisSize, eps)
	if err != nil {
		t.Fatalf("RMSNormResidualBF16Into(nil): %v", err)
	}
	if !bytes.Equal(fromNil, want) {
		t.Fatal("RMSNormResidualBF16Into(nil) output differs from allocating wrapper")
	}

	short := make([]byte, 0, axisSize*bf16Size-1)
	fromShort, err := RMSNormResidualBF16Into(short, x, w, res, axisSize, eps)
	if err != nil {
		t.Fatalf("RMSNormResidualBF16Into(short): %v", err)
	}
	if len(fromShort) != axisSize*bf16Size {
		t.Fatalf("RMSNormResidualBF16Into(short) returned %d bytes, want %d", len(fromShort), axisSize*bf16Size)
	}
	if !bytes.Equal(fromShort, want) {
		t.Fatal("RMSNormResidualBF16Into(short) output differs from allocating wrapper")
	}
}
