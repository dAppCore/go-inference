...
// heuristic_scan.go — a byte-identical replacement for the count-only
// `(?i)\b(?:literal|literal|…)\b` alternations (complianceCombined,
// emotionCombined, complianceOverlap). RE2 runs a full (?i)+\b DFA over the
// input for every FindAllStringIndex; a direct multi-literal word-boundary
// scan does the same work in one cheaper pass. It stays byte-identical by
// building on the SAME two primitives RE2 uses:
//
...
//     fold cycle (so 's' matches ſ U+017F, 'k' matches K U+212A, exactly as
//     the regex does — verified interior-fold case "kindneſs" matches
//     \bkindness\b).
...
//     the boundary decided by that rune's non-word-ness — again as the regex.
//
// FindAllStringIndex counts LEFTMOST, NON-OVERLAPPING matches, so the scan
// takes the first phrase (in alternation order) that matches with both
// boundaries at each position and resumes past it. The
// TestHeuristicScan_* differential fuzz oracle pins parity against the live
// regexps across an alphabet seeded with the fold hazards.
...
// phraseSet is a compiled alternation of lower-case ASCII literals, ready for
// the leftmost-non-overlapping count. Phrases are bucketed by their (folded)
// first letter so a scan position tries only the handful of phrases that can
// begin there instead of the whole set — turning the per-position cost from
...
// newPhraseSet compiles phrases (all lower-case ASCII, first char a letter)
// into a scan-ready set, preserving alternation order within each first-letter
// bucket so leftmost-first is honoured.
func newPhraseSet(phrases []string) *phraseSet {
	ps := &phraseSet{phrases: phrases}
...
func (ps *phraseSet) count(text string) int {
	count := 0
	pos := 0
	for pos < len(text) {
		b := text[pos]
...
var (
	complianceSet        = newPhraseSet(compliancePhrases)
	complianceOverlapSet = newPhraseSet(complianceOverlapPhrases)
	emotionSet           = newPhraseSet(emotionPhrases)
)
