...
// throughout this codebase's safetensors-adjacent readers.
var (
	errDecodeF32PayloadMismatch  = core.NewError("modelmgmt: F32 payload length does not match element count")
	errDecodeF16PayloadMismatch  = core.NewError("modelmgmt: F16 payload length does not match element count")
	errDecodeBF16PayloadMismatch = core.NewError("modelmgmt: BF16 payload length does not match element count")
	errDecodeF64PayloadMismatch  = core.NewError("modelmgmt: F64 payload length does not match element count")
)
...
func DecodeFloat32(dtype string, raw []byte, elements int) ([]float32, error) {
	...
	default:
		return nil, core.NewError("modelmgmt: unsupported safetensors dtype for float decode: " + dtype)
	}
	return values, nil
}
