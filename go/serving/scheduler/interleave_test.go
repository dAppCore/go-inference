...
// fakeSource builds a source that reports to started (when non-nil) the instant
// its stream begins running, then optionally blocks on release (when non-nil)
// before yielding n deterministic tokens "<id>-0".."<id>-(n-1)" — the
// controllable-timing double the admission/cancellation/backpressure tests
// drive. Shared by the interleave and batch suites.
//
...
// drainTokens reads every token off ch until it closes, with a timeout so a
// stuck test fails fast instead of hanging the suite.
func drainTokens(t *testing.T, ch <-chan inference.Token) []string {
	t.Helper()
	var got []string
	...
	for {
		...
		case <-deadline:
			t.Fatalf("drainTokens: timed out, got %v so far", got)
		}
	}
}

// waitStats polls get() until want reports true or the deadline passes — a run
// loop's bookkeeping updates asynchronously to a retirement/close, so tests
// that assert on it need this rather than an immediate read.
func waitStats(t *testing.T, get func() Stats, want func(Stats) bool) Stats {
	t.Helper()
	deadline := time.After(5 * time.Second)
	for {
		s := get()
...
func TestInterleave_Submit_Good(t *testing.T) {
	e := newInterleaveEngine(Config{MaxConcurrent: 4, MaxQueue: 4, StreamBuffer: 4})
	defer e.close()
	...
	waitStats(t, e.stats, func(s Stats) bool { return s.Completed == 1 && s.Active == 0 })
}

func TestInterleave_Submit_Bad(t *testing.T) {
	e := newInterleaveEngine(Config{})
	defer e.close()
...
func TestInterleave_Submit_Ugly(t *testing.T) {
	// MaxActive: 1 occupied by a request that never releases, MaxQueue: 1 filled
	// by a second request that can never be admitted either — a third submit has
	// nowhere to go, so it must block on the caller's own ctx rather than hang.
	e := newInterleaveEngine(Config{MaxConcurrent: 1, MaxQueue: 1, StreamBuffer: 1})
	defer e.close()
...
func TestInterleave_Cancel_Good(t *testing.T) {
	e := newInterleaveEngine(Config{MaxConcurrent: 4, MaxQueue: 4, StreamBuffer: 4})
	defer e.close()
...
func TestInterleave_Cancel_Bad(t *testing.T) {
	// MaxActive: 1 occupied by a blocked request, so a second submit sits queued
	// — cancel on the QUEUED one must retire it without ever running its source.
	e := newInterleaveEngine(Config{MaxConcurrent: 1, MaxQueue: 4, StreamBuffer: 4})
	defer e.close()
...
func TestInterleave_Cancel_Ugly(t *testing.T) {
	e := newInterleaveEngine(Config{MaxConcurrent: 1, MaxQueue: 1, StreamBuffer: 1})
	defer e.close()
...
func TestInterleave_Submit_MaxActive(t *testing.T) {
	e := newInterleaveEngine(Config{MaxConcurrent: 1, MaxQueue: 4, StreamBuffer: 4})
	defer e.close()
...
func TestInterleave_Submit_MaxBatchTokens(t *testing.T) {
	e := newInterleaveEngine(Config{MaxConcurrent: 4, MaxQueue: 4, MaxBatchTokens: 100, StreamBuffer: 4})
	defer e.close()
...
func TestInterleave_Submit_BackpressureIsolation(t *testing.T) {
	e := newInterleaveEngine(Config{MaxConcurrent: 4, MaxQueue: 4, StreamBuffer: 1})
	defer e.close()
	...
	// Cleanup: cancel the still-blocked slow request so close doesn't wait out
	// its source.
	e.cancel("slow")
	drainTokens(t, slowTokens)
}

func TestInterleave_Close_Good(t *testing.T) {
	e := newInterleaveEngine(Config{MaxConcurrent: 1, MaxQueue: 4, StreamBuffer: 4})
