...
// The Quantize benches baseline the GGUF quantisation kernels (AX-11) through the ONE public
// entry every engine streams weights through — one bench per format, so each of the nine
// kernels (the _0 family over 32-value blocks, the K family over 256-value super-blocks) is
// measured at a realistic per-tensor size. The kernel is the whole cost; the packed output is
// the single allocation. This covers quantize_kernels.go's kernels via the dispatch entry (no
// separate direct-kernel benches — that would double-bench the same symbols). Pure Go,
// synthetic values — no file.
