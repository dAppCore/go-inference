...
// The quantize benches baseline the format-resolution surface of quantize.go (AX-11), all per
...
// bytes-per-block); ValidationSummary joins issue codes for a failure report. The kernels
// themselves are benched via the public Quantize entry in quantize_dispatch_bench_test.go
// (Quantize/AppendQuantize live in quantize_dispatch.go). Pure lookups + small string joins —
// no file, no kernel.
