...
func (c *Config) Arch() (model.Arch, error) {
	...
	// Qwen2/2.5's feed-forward is SwiGLU (hidden_act "silu"); default to it when the config omits
	// the field. The decode reads Arch.Activation to pick SiLU over the gemma-family GELU gate.
	...
	return model.Arch{
		...
		FF: c.IntermediateSize, Vocab: c.VocabSize, Eps: eps, Activation: activation,
