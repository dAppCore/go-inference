...
// TestFactoryWeightNames_Good pins the Qwen2-MoE/Qwen3-MoE factory tensor layout: the llama/qwen 2-norm
// coupling (post_attention_layernorm is the pre-MoE norm, no gemma-style sandwich norm), the router and
// packed-expert role names, the shared-expert names, and that the standard attention names are kept from
// StandardWeightNames.
func TestFactoryWeightNames_Good(t *testing.T) {
	...
	if w.PostAttnNorm != "" {
		t.Errorf("PostAttnNorm = %q, want \"\" — a non-empty value would apply a second (gemma-style) norm qwenmoe does not carry", w.PostAttnNorm)
	}
	if w.NormBiasOne {
		t.Error("NormBiasOne = true, want false (qwenmoe is plain RMSNorm, not gemma's +1 fold)")
	}
	...
	// Qwen3-MoE ships self_attn.q_norm/k_norm (Qwen2-MoE does not) — StandardWeightNames' defaults
	// already name these; qwenmoe leaves them untouched rather than overriding per generation, relying on
	// LoadLinear's nil-safe absence handling (mirrors gptoss.WeightNames' doc for the same situation).
	if w.QNorm != ".self_attn.q_norm.weight" || w.KNorm != ".self_attn.k_norm.weight" {
		t.Errorf("QNorm/KNorm = %q/%q, want the StandardWeightNames defaults (nil-safe on Qwen2-MoE, present on Qwen3-MoE)", w.QNorm, w.KNorm)
	}
}

// TestFactoryWeightNames_Ugly proves the router, packed-expert, and shared-expert names never collide
// with each other — a collision would silently alias two roles onto one tensor.
func TestFactoryWeightNames_Ugly(t *testing.T) {
	w := FactoryWeightNames()
	names := map[string]string{
		"router": w.MoE.Router, "gate": w.MoE.ExpGate, "up": w.MoE.ExpUp, "down": w.MoE.ExpDown,
		"sharedGate": w.MoE.SharedGate, "sharedUp": w.MoE.SharedUp, "sharedDown": w.MoE.SharedDown, "sharedSigmoid": w.MoE.SharedSigmoid,
	}
...
// expertCheckpoint builds a minimal 2-layer, numExperts-expert Qwen-MoE tensor set: every expert's
// gate/up/down_proj is a distinct, deterministically-seeded tensor (seed = layer*100 + expert*10 + role)
...
// of the source per-expert tensors, in expert-index order, for every layer and every role.
...
// rejected before any lookup (never a nonsensical zero-sized allocation or an infinite loop), and a nil
