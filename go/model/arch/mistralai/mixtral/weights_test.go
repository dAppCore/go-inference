...
// TestFactoryWeightNames_Good pins the Mixtral factory tensor layout: the llama/mistral 2-norm coupling
// (post_attention_layernorm is the pre-MoE norm, no gemma-style sandwich norm), the router name, the
// packed-expert role names, and that the standard attention names are kept from StandardWeightNames.
func TestFactoryWeightNames_Good(t *testing.T) {
	...
	if w.PostAttnNorm != "" {
		t.Errorf("PostAttnNorm = %q, want \"\" — a non-empty value would apply a second (gemma-style) norm Mixtral does not carry", w.PostAttnNorm)
	}
	if w.NormBiasOne {
		t.Error("NormBiasOne = true, want false (Mixtral is plain RMSNorm, not gemma's +1 fold)")
	}

	if w.MoE.Router != ".block_sparse_moe.gate" {
		t.Errorf("MoE.Router = %q, want .block_sparse_moe.gate", w.MoE.Router)
	}
...
// TestFactoryWeightNames_Bad guards against Mixtral acquiring a shared expert by accident — real Mixtral
// checkpoints carry no shared_expert.* tensors, so a stray non-empty name here would make assembleMoE
// probe a tensor that never exists (harmless — LoadLinear is nil-safe on a missing name — but a silent
// declaration drift worth pinning). QNorm/KNorm are deliberately NOT asserted empty here: Mixtral has no
// QK-norm either, but StandardWeightNames' default names simply never match a real checkpoint tensor and
// nil-safe out, so leaving them (rather than clearing) is the same zero-cost convention gptoss.WeightNames
// uses.
func TestFactoryWeightNames_Bad(t *testing.T) {
	w := FactoryWeightNames()
	if w.MoE.SharedGate != "" || w.MoE.SharedUp != "" || w.MoE.SharedDown != "" || w.MoE.SharedSigmoid != "" {
		t.Fatalf("Mixtral declared a shared expert it does not have: %+v", w.MoE)
	}
}
...
// bf16Tensor2D builds a deterministic 2-D bf16 tensor whose bytes encode (row, col) so packExperts'
...
// expertCheckpoint builds a minimal 2-layer, numExperts-expert Mixtral tensor set: every expert's w1/w2/w3
// is a distinct, deterministically-seeded tensor (seed = layer*100 + expert*10 + role) so packExperts'
// concatenation ORDER is provable, not just its total byte count.
func expertCheckpoint(numLayers, numExperts, ff, hidden int) map[string]safetensors.Tensor {
	...
	for layer := 0; layer < numLayers; layer++ {
		for e := 0; e < numExperts; e++ {
			...
			prefix := core.Sprintf("model.layers.%d.block_sparse_moe.experts.%d.", layer, e)
			out[prefix+"w1.weight"] = bf16Tensor2D(ff, hidden, base+1) // gate
			out[prefix+"w3.weight"] = bf16Tensor2D(ff, hidden, base+2) // up
			out[prefix+"w2.weight"] = bf16Tensor2D(hidden, ff, base+3) // down
		}
	}
	return out
}
...
func TestPackExperts_Good(t *testing.T) {
	...
	for layer := 0; layer < numLayers; layer++ {
		for _, role := range expertRoles {
			...
			for e := 0; e < numExperts; e++ {
				src := in[core.Sprintf("model.layers.%d.block_sparse_moe.experts.%d.%s.weight", layer, e, role.hfSuffix)]
				want = append(want, src.Data...)
				outDim = src.Shape[0]
				inDim = src.Shape[1]
			}
			packedName := core.Sprintf("model.layers.%d%s.%s.weight", layer, packedExpertsPrefix, role.packedSuffix)
			packed, ok := out[packedName]
			if !ok {
				t.Fatalf("layer %d role %s: packed tensor %q absent", layer, role.packedSuffix, packedName)
			}
			if !bytes.Equal(packed.Data, want) {
				t.Fatalf("layer %d role %s: packed bytes != concatenation of experts 0..%d in order", layer, role.packedSuffix, numExperts-1)
			}
			if len(packed.Shape) != 2 || packed.Shape[0] != outDim*numExperts || packed.Shape[1] != inDim {
				t.Fatalf("layer %d role %s: packed shape = %v, want [%d,%d]", layer, role.packedSuffix, packed.Shape, outDim*numExperts, inDim)
			}
			if packed.Dtype != "BF16" {
				t.Fatalf("layer %d role %s: packed dtype = %q, want BF16", layer, role.packedSuffix, packed.Dtype)
			}
		}
	}
}
...
// from Mixtral's per-expert quant triples so a quantised Mixtral checkpoint can serve too (#59).
func TestPackExperts_QuantisedScalesAndBiases_Good(t *testing.T) {
	...
	for e := 0; e < numExperts; e++ {
		...
		in[prefix+"w1.scales"], in[prefix+"w1.biases"] = bf16Tensor2D(ff, groups, base+201), bf16Tensor2D(ff, groups, base+202)
		in[prefix+"w3.scales"], in[prefix+"w3.biases"] = bf16Tensor2D(ff, groups, base+203), bf16Tensor2D(ff, groups, base+204)
		in[prefix+"w2.scales"], in[prefix+"w2.biases"] = bf16Tensor2D(hidden, groups, base+205), bf16Tensor2D(hidden, groups, base+206)
	}
	...
	for _, role := range expertRoles {
		...
		var outDim int
		for e := 0; e < numExperts; e++ {
			sSrc := in[core.Sprintf("model.layers.0.block_sparse_moe.experts.%d.%s.scales", e, role.hfSuffix)]
			bSrc := in[core.Sprintf("model.layers.0.block_sparse_moe.experts.%d.%s.biases", e, role.hfSuffix)]
...
func TestPackExperts_QuantisedScalesAndBiases_Bad(t *testing.T) {
	t.Run("biases_absent", func(t *testing.T) {
		...
		in["model.layers.0.block_sparse_moe.experts.0.w1.scales"] = bf16Tensor2D(8, 2, 1)
	...
	t.Run("scales_shape_mismatch", func(t *testing.T) {
		...
		for _, e := range []int{0, 1} {
			prefix := core.Sprintf("model.layers.0.block_sparse_moe.experts.%d.", e)
			in[prefix+"w1.scales"] = bf16Tensor2D(8, 2, uint16(e))
			in[prefix+"w1.biases"] = bf16Tensor2D(8, 2, uint16(e))
		}
		in["model.layers.0.block_sparse_moe.experts.1.w1.scales"] = bf16Tensor2D(8, 3, 9) // wrong group count
		if _, err := packExperts(in, 1, 2); err == nil {
...
func TestPackExperts_Bad(t *testing.T) {
	...
	t.Run("dtype_mismatch", func(t *testing.T) {
		in := expertCheckpoint(1, 2, 8, 4)
		bad := in["model.layers.0.block_sparse_moe.experts.1.w1.weight"]
		bad.Dtype = "F32"
		in["model.layers.0.block_sparse_moe.experts.1.w1.weight"] = bad
		if _, err := packExperts(in, 1, 2); err == nil {
...
// TestPackExperts_Ugly covers the degenerate inputs: a non-positive layer or expert count must be
// rejected before any lookup (never a nonsensical zero-sized allocation or an infinite loop), and an
// empty-but-non-nil input map degrades gracefully.
