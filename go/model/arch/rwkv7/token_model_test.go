...
func TestTokenModel_NewTokenModel_Bad(t *testing.T) {
	tm := NewTokenModel(&RWKV7Model{})
	if tm.Vocab() != 0 {
		t.Fatalf("Vocab = %d, want 0 for a zero-value model", tm.Vocab())
	}
}

// TestTokenModel_NewTokenModel_Ugly proves two wrappers over the SAME model are independently usable and
// deterministic — Embed on either produces identical bytes.
...
func TestTokenModel_RWKV7TokenModel_Vocab_Good(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	m := mkRWKV7Model(cfg, 8, 16, 32, 1)
...
// TestTokenModel_RWKV7TokenModel_Vocab_Ugly proves Vocab() reads the underlying model LIVE, not a
// snapshot taken at NewTokenModel time.
func TestTokenModel_RWKV7TokenModel_Vocab_Ugly(t *testing.T) {
	m := &RWKV7Model{Vocab: 10}
	tm := NewTokenModel(m)
	m.Vocab = 99
...
func TestTokenModel_RWKV7TokenModel_Embed_Bad(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	m := mkRWKV7Model(cfg, 8, 16, 32, 1)
	tm := NewTokenModel(m)
...
// TestTokenModel_RWKV7TokenModel_Embed_Ugly pins the boundary: id == Vocab-1 (the LAST valid id) must be
// accepted — distinct from _Bad's id == Vocab (one past) rejection.
func TestTokenModel_RWKV7TokenModel_Embed_Ugly(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	m := mkRWKV7Model(cfg, 8, 16, 32, 1)
	tm := NewTokenModel(m)
...
func TestTokenModel_RWKV7TokenModel_Head_Good(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	m := mkRWKV7Model(cfg, 8, 16, 32, 1)
	tm := NewTokenModel(m)
...
func TestTokenModel_RWKV7TokenModel_Head_Bad(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	m := mkRWKV7Model(cfg, 8, 16, 32, 1)
	tm := NewTokenModel(m)
...
// TestTokenModel_RWKV7TokenModel_Head_Ugly proves the tied-vs-explicit LM head branch takes effect.
func TestTokenModel_RWKV7TokenModel_Head_Ugly(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	tied := mkRWKV7Model(cfg, 8, 16, 32, 1)
	explicit := mkRWKV7Model(cfg, 8, 16, 32, 1)
...
func TestTokenModel_RWKV7TokenModel_DecodeForward_Good(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	m := mkRWKV7Model(cfg, 8, 16, 32, 2)
	tm := NewTokenModel(m)
	tokens := []int32{3, 1, 4}
...
func TestTokenModel_RWKV7TokenModel_DecodeForward_Bad(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	m := mkRWKV7Model(cfg, 8, 16, 32, 1)
	tm := NewTokenModel(m)
...
// TestTokenModel_RWKV7TokenModel_DecodeForward_Ugly proves the empty-input edge: zero inputs returns
// (nil, nil) — no error, no panic.
func TestTokenModel_RWKV7TokenModel_DecodeForward_Ugly(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	m := mkRWKV7Model(cfg, 8, 16, 32, 1)
	tm := NewTokenModel(m)
...
func TestTokenModel_RWKV7TokenModel_OpenSession_Bad(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	m := mkRWKV7Model(cfg, 8, 16, 32, 1)
...
// TestTokenModel_RWKV7TokenModel_OpenSession_Ugly proves session independence: two steppers opened from
// the same model both start from FRESH (zero) state — stepping the identical token through each produces
// identical output, proving OpenSession doesn't leak state between calls.
func TestTokenModel_RWKV7TokenModel_OpenSession_Ugly(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	m := mkRWKV7Model(cfg, 8, 16, 32, 2)
	tm := NewTokenModel(m)
	e, err := tm.Embed(7)
...
// TestTokenModel_RWKV7TokenModel_OpenSession_Good verifies the SessionModel contract: decoding token by
// token through OpenSession's incremental stepper produces the SAME hidden bytes as the whole-sequence
// DecodeForward — what lets Generate prefer the O(1)/token fast path with no change in output.
func TestTokenModel_RWKV7TokenModel_OpenSession_Good(t *testing.T) {
	cfg := BlockConfig{NumHeads: 2, KeyDim: 4, ValueDim: 3}
	m := mkRWKV7Model(cfg, 8, 16, 32, 2)
	tm := NewTokenModel(m)
	...
	t.Logf("rwkv7 SessionModel: incremental decode == whole-sequence over %d tokens (bf16 seam consistent)", len(tokens))
}
