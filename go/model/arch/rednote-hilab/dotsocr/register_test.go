...
// the arch is, and both the Arch and Composed refusal seams name the arch that actually resolved
// and say why the forward is missing — mirroring composed.TestComposedMTPRefusal's discipline
...
func TestDotsocrRegistered_Good(t *testing.T) {
	for _, mt := range []string{"dots_ocr", "dots_ocr_1_5"} {
		...
		tm, err := spec.Composed(map[string]safetensors.Tensor{}, cfgJSON)
		if err == nil {
			t.Fatalf("%q: Composed: expected a clean forward refusal, got a model", mt)
		}
		if tm != nil {
			t.Fatalf("%q: Composed: refusal must return a nil model", mt)
		}
		if !core.Contains(err.Error(), mt) || !core.Contains(err.Error(), "not yet implemented") {
			t.Fatalf("%q: Composed refusal %q must name the arch and say the forward is not yet implemented", mt, err.Error())
		}
	}
}
