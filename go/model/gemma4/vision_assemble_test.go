...
func TestAssembleVisionQuantizedEncoderWeightsGood(t *testing.T) {
	...
	tower, err := AssembleVision(weights, tc)
	if err != nil {
	...
	assertAffine := func(label string, linear vision.Linear, outDim, inDim int) {
		t.Helper()
	...
	assertAffine("patch", tower.PatchProjection, hidden, patchDim)
	assertAffine("q", tower.Layers[0].Q, hidden, hidden)
	assertAffine("k", tower.Layers[0].K, hidden, hidden)
	assertAffine("v", tower.Layers[0].V, hidden, hidden)
	assertAffine("o", tower.Layers[0].O, hidden, hidden)
	assertAffine("gate", tower.Layers[0].Gate, ffDim, hidden)
	assertAffine("up", tower.Layers[0].Up, ffDim, hidden)
	assertAffine("down", tower.Layers[0].Down, hidden, ffDim)
	assertAffine("projector", tower.Projector.Projection, hidden, hidden)
}
