...
// features.go is the engine-neutral mel feature extractor: raw 16 kHz waveform → the log-mel
// input_features the Conformer encoder consumes (Mantis #1839). It is a pure HOST-side port of the
// engine/metal native extractor — float64 radix-2 rfft + HTK triangular mel filterbank — and produces
// the same bytes (the FFT is host, NOT a GPU radix kernel, so there is no ABI to match). Engine-neutral:
// no model name; geometry arrives as FeatureConfig.
//
// Pipeline (ported from HF transformers Gemma4AudioFeatureExtractor step by step): truncate → pad to a
// sample multiple → semicausal prepend (frame/2 zeros) → unfold frames (frame+1 window, hop stride) →
// periodic Hann → rfft → magnitude → HTK triangular mel bank → log(mel + floor) → frame-validity mask
// (a frame is real only when its window's last sample is real audio), with masked frames zeroed. The
// float64 pipeline mirrors numpy's promotion and casts to float32 at the end.

// FeatureConfig mirrors the feature_extractor section of the model's processor_config.json. The model
// is the source of truth — absent dimensions stay zero and fail loud at extractor build.
type FeatureConfig struct {
	...
	// Converted snapshots vary in key spelling: mlx-community ships num_mel_filters for the mel count
	// and ms-based frame/hop fields may appear instead of sample counts. Aliases resolve in normalisation.
...
// processorConfig is the slice of processor_config.json this package reads (the image/video sections
// belong to the vision lane).
...
// LoadFeatureConfig reads the audio feature_extractor section from the model directory's
// processor_config.json. Returns (nil, nil) when the model ships no processor config (text-only
// checkpoints). Reads via the core helpers, not the banned stdlib os/encoding/json.
func LoadFeatureConfig(modelPath string) (*FeatureConfig, error) {
...
// FeatureExtractor converts waveforms to log-mel features.
type FeatureExtractor struct {
	cfg        *FeatureConfig
	window     []float32 // periodic Hann over FrameLength
	melFilters [][]float64
	// HF __call__ defaults: clips truncate at 30 s (480 000 samples @16k) and waveforms right-pad to a
...
func (e *FeatureExtractor) SamplingRate() int32 {
	if e == nil {
		return 0
	}
...
// normalizeFeatureConfig resolves alias keys and fills absent fields with the HF
// Gemma4AudioFeatureExtractor constructor defaults (feature_extraction_gemma4.py) — published spec, not
// invention. Converted snapshots ship partial sections (mlx-community: sampling_rate + hop_length +
// num_mel_filters only); the HF loader fills the rest the same way.
func normalizeFeatureConfig(cfg *FeatureConfig) *FeatureConfig {
	if cfg == nil {
		return nil
	}
...
// NewFeatureExtractor builds the extractor from the model's declared feature config (absent fields take
// the HF constructor defaults via normalisation). Fails loud on a non-power-of-two FFT length (the rfft
// below is radix-2) or a contradictory mel band.
func NewFeatureExtractor(cfg *FeatureConfig) (*FeatureExtractor, error) {
	if cfg == nil {
		return nil, core.NewError("audio: feature config is nil")
	}
	resolved := *cfg
	normalizeFeatureConfig(&resolved)
	...
	if fft&(fft-1) != 0 || fft < resolved.FrameLength {
		return nil, core.E("audio", core.Sprintf("fft_length %d must be a power of two ≥ frame_length %d", fft, resolved.FrameLength), nil)
	}
	if resolved.MaxFrequency <= resolved.MinFrequency {
		return nil, core.E("audio", core.Sprintf("mel band [%v, %v] is empty", resolved.MinFrequency, resolved.MaxFrequency), nil)
	}
	resolved.FFTLength = fft

	// Periodic Hann, float32 like the reference (frames multiply in f32 before numpy's rfft promotes to
	// f64 — kept bit-faithful).
...
// Extract converts one waveform (16 kHz mono, [-1,1] float32 samples) into log-mel features. Returns
// the features as a flat [frames × FeatureSize] float32 slice, the per-frame validity mask, and the
// frame count.
func (e *FeatureExtractor) Extract(samples []float32) ([]float32, []bool, int, error) {
	if e == nil {
		return nil, nil, 0, core.NewError("audio: feature extractor is nil")
	}
	if len(samples) == 0 {
		return nil, nil, 0, core.NewError("audio: empty waveform")
	}
	cfg := e.cfg
	...
	// Semicausal prepend (frame/2 zeros) so the first frame centres at t=0. The waveform buffer carries
	// [prepend ⊕ samples ⊕ right-pad]; validity marks only the real samples.
	...
	if numFrames <= 0 {
		return nil, nil, 0, core.E("audio", core.Sprintf("waveform too short: %d samples < frame %d", realLen, frameSize), nil)
	}
	if cfg.Preemphasis != 0 {
		return nil, nil, 0, core.NewError("audio: preemphasis extraction not implemented (no shipped Gemma 4 config uses it)")
	}
	...
	for f := 0; f < numFrames; f++ {
		...
		// A frame is real audio only when its window's LAST sample is — masked frames zero out,
		// mirroring the reference's mask multiply.
...
// InputFeatures converts one mono waveform through the host extractor and returns the bf16 [frames,
// melBins] rows consumed by the audio encoder, the frame count, and the mel-bin count.
func InputFeatures(samples []float32, extractor *FeatureExtractor) ([]byte, int, int, error) {
	features, _, frames, err := extractor.Extract(samples)
	if err != nil {
		return nil, 0, 0, err
	}
	if frames <= 0 || len(features)%frames != 0 {
		return nil, 0, 0, core.NewError("audio.InputFeatures: invalid audio feature geometry")
	}
...
func htkMelFilterBank(bins, numMel int, minFreq, maxFreq float64, samplingRate int) [][]float64 {
	...
	// linspace(0, samplingRate//2, bins) — integer-divided ceiling per the reference (matters only for
	// odd sampling rates).
...
// rfft computes an in-place iterative radix-2 FFT of the real input frame into spectrum (full complex
// spectrum; callers read bins [0, n/2]).
func rfft(frame []float64, spectrum []complex128) {
	n := len(frame)
