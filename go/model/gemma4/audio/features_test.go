...
// features_test.go pins the mel front-end's DSP internals (rfft, htkMelFilterBank, config
// normalisation, error branches) at the unit level — ported from engine/metal's audio_features_test.go
// as part of the #44 convergence (the front-end is now owned here; engine/metal delegates via a type
// alias, see engine/metal/audio_features.go).

func TestFeatureConfigLoadAndNormalize(t *testing.T) {
	dir := t.TempDir()
	data := []byte(`{
	...
	cfg, err := LoadFeatureConfig(dir)
	if err != nil {
		t.Fatalf("LoadFeatureConfig: %v", err)
	}
	if cfg == nil {
		t.Fatal("LoadFeatureConfig returned nil config")
	}
	...
	missing, err := LoadFeatureConfig(t.TempDir())
	if err != nil {
		t.Fatalf("LoadFeatureConfig(missing): %v", err)
	}
	if missing != nil {
...
func TestFeatureExtractorExtractMasksPaddedFrames(t *testing.T) {
	extractor, err := NewFeatureExtractor(&FeatureConfig{
		NumMelFilters:    4,
		SamplingRate:     16_000,
		FrameLength:      4,
		HopLength:        2,
	...
	if got := (*FeatureExtractor)(nil).SamplingRate(); got != 0 {
...
func TestInputFeatures_Good(t *testing.T) {
	extractor, err := NewFeatureExtractor(&FeatureConfig{
		NumMelFilters: 4,
		SamplingRate:  16_000,
		FrameLength:   4,
	...
	got, frames, melBins, err := InputFeatures(samples, extractor)
	if err != nil {
		t.Fatalf("InputFeatures: %v", err)
	}
	...
	if !slices.Equal(got, f32ToBf16Slice(wantF32)) {
		t.Fatal("InputFeatures did not return bf16-converted extractor rows")
	}
}

func TestFeatureExtractorErrorBranches(t *testing.T) {
	if _, err := NewFeatureExtractor(nil); err == nil {
		t.Fatal("NewFeatureExtractor(nil) error = nil")
	}
	if _, err := NewFeatureExtractor(&FeatureConfig{
		FeatureSize:  1,
		SamplingRate: 16_000,
		FrameLength:  8,
		HopLength:    2,
		...
		t.Fatal("NewFeatureExtractor(non-power-of-two FFT) error = nil")
	}
	if _, err := NewFeatureExtractor(&FeatureConfig{
		FeatureSize:  1,
		SamplingRate: 16_000,
		...
		t.Fatal("NewFeatureExtractor(empty mel band) error = nil")
	}
	...
	if err != nil {
		t.Fatalf("NewFeatureExtractor(preemphasis config): %v", err)
	}
	if _, _, _, err := extractor.Extract([]float32{0.1, 0.2, 0.3, 0.4}); err == nil {
		t.Fatal("Extract(preemphasis) error = nil")
	}
	if _, _, _, err := (*FeatureExtractor)(nil).Extract([]float32{0.1}); err == nil {
