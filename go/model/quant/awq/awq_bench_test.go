...
func BenchmarkQuantize(b *testing.B) {
	...
	b.ResetTimer()
	for range b.N {
		benchmarkTensor, _ = Quantize(values, 256, 256, Options{Bits: 4, GroupSize: 128, ZeroPoint: true})
	}
}

func BenchmarkDequantize(b *testing.B) {
	values := make([]float32, 256*256)
	tensor, _ := Quantize(values, 256, 256, Options{Bits: 4, GroupSize: 128, ZeroPoint: true})
	b.ReportAllocs()
	b.ResetTimer()
	for range b.N {
