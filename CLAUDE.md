...
- **What keeps `model/composed` alive** (the #18/#50 deletion checklist): engine/hip's consumer (codex-owned until ~Aug 10), the arch-zoo Composed registrations still WITHOUT a factory route (dbrx/olmoe/granitemoe/llama4/qwenmoe + the generic composed/hybrid ids), the qwen MTP pair + vision tower, mamba2/rwkv7. **qwen3_6/qwen3_6_moe/qwen3_next now register in model/arch/Qwen/qwen35 (same dual-route ArchSpec as qwen3_5 — #50 archzoo) and mixtral has a real factory port (model/arch/mistralai/mixtral, packed-expert synthesis at load) — none of the three REQUIRES composed to load any more**, but engine/metal's serve-time default (`isQwen35FactoryType` in `engine/metal/load.go`) still only lists qwen3_5*, so `lem serve`/`generate` keeps routing these three through composed today; extending that switch (+ a live A/B, mirroring qwen3_5's `237db3a1` flip) is the follow-up, not a load-bearing blocker. Next gap to close: mlx-lm ~110 tok/s on the 35B vs our 30 — now a unified-engine campaign.
